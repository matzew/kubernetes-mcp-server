...
# Configure cross-realm token exchange trust for a managed cluster
# Usage: make acm-setup-cross-realm-trust MANAGED_KUBECONFIG=<path> MANAGED_ADMIN_USER=<user> MANAGED_ADMIN_PASSWORD=<pass>
acm-setup-cross-realm-trust: ## Configure cross-realm token exchange trust for a managed cluster
	@if [ -z "$(MANAGED_KUBECONFIG)" ]; then \
		echo "Error: MANAGED_KUBECONFIG is required"; \
		echo "Usage: make acm-setup-cross-realm-trust MANAGED_KUBECONFIG=<path> MANAGED_ADMIN_USER=<user> MANAGED_ADMIN_PASSWORD=<pass>"; \
		exit 1; \
	fi
	@if [ ! -f "$(MANAGED_KUBECONFIG)" ]; then \
		echo "Error: Kubeconfig file not found: $(MANAGED_KUBECONFIG)"; \
	...
	@if [ -z "$(MANAGED_ADMIN_USER)" ] || [ -z "$(MANAGED_ADMIN_PASSWORD)" ]; then \
		echo "Error: MANAGED_ADMIN_USER and MANAGED_ADMIN_PASSWORD are required"; \
		echo "Usage: make acm-setup-cross-realm-trust MANAGED_KUBECONFIG=<path> MANAGED_ADMIN_USER=<user> MANAGED_ADMIN_PASSWORD=<pass>"; \
		exit 1; \
	fi
	@echo "==========================================="
	@echo "Setting up Cross-Realm Token Exchange Trust"
	@echo "==========================================="
	@echo ""
	@echo "Step 1: Getting Keycloak URL from managed cluster..."
	@MANAGED_KEYCLOAK_URL=$$(KUBECONFIG=$(MANAGED_KUBECONFIG) oc get route -n keycloak keycloak -o jsonpath='https://{.spec.host}' 2>/dev/null); \
	if [ -z "$$MANAGED_KEYCLOAK_URL" ]; then \
		echo "❌ Error: Keycloak route not found in managed cluster"; \
		echo "   Make sure Keycloak is installed in the 'keycloak' namespace"; \
		exit 1; \
	fi; \
	echo "✅ Found Keycloak at: $$MANAGED_KEYCLOAK_URL"; \
	echo ""; \
	echo "Step 2: Getting hub Keycloak issuer URL..."; \
	...
	echo ""; \
	echo "Step 3: Running cross-realm trust setup script..."; \
	MANAGED_KEYCLOAK_URL="$$MANAGED_KEYCLOAK_URL" \
	MANAGED_KEYCLOAK_ADMIN_USER="$(MANAGED_ADMIN_USER)" \
	MANAGED_KEYCLOAK_ADMIN_PASSWORD="$(MANAGED_ADMIN_PASSWORD)" \
